...
El chat con libros y el modo RAG usan el modelo `gemini-2.0-flash` y embeddings
`text-embedding-004` de Google. Requiere `GEMINI_API_KEY` en el backend
(`apps/api/.env`):
...
Sin esta clave, el endpoint `/api/v1/ai/chat` responde **503**
